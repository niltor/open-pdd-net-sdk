...
public async Task<GetRefundAddressListApiResult> GetRefundAddressListAsync(string Type)
{
    ...
    var result = await PostAsync<GetRefundAddressListApiResult>("pdd.refund.address.list.get",dic);
    return result;
}/// <summary>
/// 售后列表接口
/// </summary>
...
dic.Add("page",Page);
    ...
    var result = await PostAsync<GetRefundListIncrementApiResult>("pdd.refund.list.increment.get",dic);
    return result;
}/// <summary>
/// 售后校验接口
/// </summary>
...
dic.Add("order_sns",OrderSns);
    ...
    var result = await PostAsync<CheckRefundStatusApiResult>("pdd.refund.status.check",dic);
    return result;
}
    }
}
